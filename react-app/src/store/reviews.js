...
export const thunkCreateReview = (review, listingId) => async (dispatch) => {
    const res = await csrfFetch(`/api/listings/${listingId}/createreview`, {
        method: "POST",
    ...
    if (res.ok) {
        ...
        await dispatch(thunkGetListingReviews(listingId))
        return data;
    } else {
        const errors = await res.json();
        return errors;
...
export const thunkGetListingReviews = (listingId) => async (dispatch) => {
